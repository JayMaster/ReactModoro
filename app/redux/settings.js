...
export function handleAndUpdateTimer(duration) {
	return function (dispatch, getState) {
		return setTimer(duration, getState().authentication.authedId) // that getState thing accesses the redux store
		// line above is to update firebase (see api/settings.js)
...
export function handleAndUpdateRest(duration) {
	return function (dispatch, getState) {
		return setRest(duration, getState().authentication.authedId) // that getState thing accesses the redux store
		// line above is to update firebase (see api/settings.js)
